...
Belgium<-Belgium %>%
  mutate(across(everything(), ~ textclean::replace_non_ascii(.)))
names(Belgium)<-gsub(" "," ",names(Belgium))
names(Belgium)<-trimws(iconv(names(Belgium), "UTF-8", "ASCII", sub = " "))
Belgium <- Belgium %>%
  mutate(across(where(is.character), ~ trimws(iconv(.x, "UTF-8", "ASCII", sub = " "))))
...
Belgium$`Total Fish NISP`<-str_replace(Belgium$`Total Fish NISP`,"\\.0","")
Belgium$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Belgium$`NSS unique ID`<-str_replace(Belgium$`NSS unique ID`,"\\.0","")
...
nrow(Denmark)
Denmark$`Total Fish NISP`<-str_replace(Denmark$`Total Fish NISP`,"\\.0","")
Denmark$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Denmark$`NSS unique ID`<-str_replace(Denmark$`NSS unique ID`,"\\.0","")
...
nrow(England)
England$`Total Fish NISP`<-str_replace(England$`Total Fish NISP`,"\\.0","")
England$`Total Fish NISP` %>% as.numeric() %>%
  sum()
England$`NSS unique ID`<-str_replace(England$`NSS unique ID`,"\\.0","")
...
nrow(Estonia)
Estonia$`Total Fish NISP`<-str_replace(Estonia$`Total Fish NISP`,"\\.0","")
Estonia$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Estonia$`NSS unique ID`<-str_replace(Estonia$`NSS unique ID`,"\\.0","")
...
Germany_Hol$`Total Fish NISP`<-str_replace(Germany_Hol$`Total Fish NISP`,"\\.0","")
Germany_Hol$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Germany_Hol$`NSS unique ID`<-str_replace(Germany_Hol$`NSS unique ID`,"\\.0","")
...
nrow(Germany_Hein)
Germany_Hein$`Total Fish NISP`<-str_replace(Germany_Hein$`Total Fish NISP`,"\\.0","")
Germany_Hein$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Germany_Hein$`NSS unique ID`<-str_replace(Germany_Hein$`NSS unique ID`,"\\.0","")
...
Netherlands_RomSup$`Total Fish NISP`<-str_replace(Netherlands_RomSup$`Total Fish NISP`,"\\.0","")
Netherlands_RomSup$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Netherlands_RomSup$`NSS unique ID`<-str_replace(Netherlands_RomSup$`NSS unique ID`,"\\.0","")
...
Netherlands_Rom$`Total Fish NISP`<-str_replace(Netherlands_Rom$`Total Fish NISP`,"\\.0","")
Netherlands_Rom$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Netherlands_Rom$`NSS unique ID`<-str_replace(Netherlands_Rom$`NSS unique ID`,"\\.0","")
...
nrow(Ireland)
Ireland$`Total Fish NISP`<-str_replace(Ireland$`Total Fish NISP`,"\\.0","")
Ireland$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Ireland$`NSS unique ID`<-str_replace(Ireland$`NSS unique ID`,"\\.0","")
...
nrow(Norway)
Norway$`Total Fish NISP`<-str_replace(Norway$`Total Fish NISP`,"\\.0","")
Norway$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Norway$`NSS unique ID`<-str_replace(Norway$`NSS unique ID`,"\\.0","")
...
nrow(Poland)
Poland$`Total Fish NISP`<-str_replace(Poland$`Total Fish NISP`,"\\.0","")
Poland$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Poland$`NSS unique ID`<-str_replace(Poland$`NSS unique ID`,"\\.0","")
...
nrow(Scotland)
Scotland$`Total Fish NISP`<-str_replace(Scotland$`Total Fish NISP`,"\\.0","")
Scotland$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Scotland$`NSS unique ID`<-str_replace(Scotland$`NSS unique ID`,"\\.0","")
...
nrow(Scotland_b)
Scotland_b$`Total Fish NISP`<-str_replace(Scotland_b$`Total Fish NISP`,"\\.0","")
Scotland_b$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Scotland_b$`NSS unique ID`<-str_replace(Scotland_b$`NSS unique ID`,"\\.0","")
...
nrow(Sweden)
Sweden$`Total Fish NISP`<-str_replace(Sweden$`Total Fish NISP`,"\\.0","")
Sweden$`Total Fish NISP` %>% as.numeric() %>%
  sum()
Sweden$`NSS unique ID`<-str_replace(Sweden$`NSS unique ID`,"\\.0","")

# Load Data for Taxonomic Harmonisation ####
META<-read_xlsx("S4_Taxon_Traits_Table.xlsx", col_names = TRUE)
str(META)
taxa_meta<-META
taxa_meta<-taxa_meta %>%
  mutate(across(everything(), ~ trimws(.)))
...
df_Meta<-bind_rows(list(Belgium_Meta, Denmark_Meta, 
                        England_Meta, 
...
df_Meta <-df_Meta %>% mutate_at(c('NSS unique ID','Start date CE', 'End date CE',
...
df_NISP<-bind_rows(list(Belgium_Taxa, Denmark_Taxa, 
                        England_Taxa, 
...
x<-df_NISP %>% group_by(`NSS unique ID`, NISP) %>% summarise(NISP_all = sum(NISP))
...
df_Meta <-df_Meta %>% mutate_at(c('NSS unique ID'), as.character)
...
names(all_df)
df_final<-all_df[,c(1,3,57,5:54,2,4,56,59,60:80)] %>% distinct()
df_final<-df_final[!is.na(df_final$`NSS unique ID`),]
df_final$NISP<-as.numeric(df_final$NISP)
x<-df_final %>% 
  group_by(Database) %>%
  summarise(Sum_X = sum(NISP, na.rm = TRUE))
df_final %>% 
  summarise(Sum_X = sum(NISP, na.rm = TRUE))
df_final<-df_final[,c(1:78)]
